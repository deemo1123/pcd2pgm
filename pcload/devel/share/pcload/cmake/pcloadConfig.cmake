...
if(pcload_CONFIG_INCLUDED)
  return()
endif()
set(pcload_CONFIG_INCLUDED TRUE)

# set variables for source/devel/install prefixes
if("TRUE" STREQUAL "TRUE")
  set(pcload_SOURCE_PREFIX /home/oem/pcd2pgm/pcload/src/pcload)
  set(pcload_DEVEL_PREFIX /home/oem/pcd2pgm/pcload/devel)
  set(pcload_INSTALL_PREFIX "")
  set(pcload_PREFIX ${pcload_DEVEL_PREFIX})
else()
  set(pcload_SOURCE_PREFIX "")
  set(pcload_DEVEL_PREFIX "")
  set(pcload_INSTALL_PREFIX /home/oem/pcd2pgm/pcload/install)
  set(pcload_PREFIX ${pcload_INSTALL_PREFIX})
endif()

# warn when using a deprecated package
if(NOT "" STREQUAL "")
  set(_msg "WARNING: package 'pcload' is deprecated")
...
set(pcload_FOUND_CATKIN_PROJECT TRUE)
...
if(NOT "/usr/include/pcl-1.10;/usr/include/eigen3;/usr/include;/usr/include/ni;/usr/include/openni2;/usr/include/vtk-7.1;/usr/include/freetype2;/usr/include/x86_64-linux-gnu " STREQUAL " ")
  ...
  elseif(NOT " " STREQUAL " ")
    ...
    set(_report "Report the problem to the maintainer 'Igor Smirnov <smirnoviv@rambler.ru>' and request to fix the problem.")
  endif()
  foreach(idir ${_include_dirs})
    if(IS_ABSOLUTE ${idir} AND IS_DIRECTORY ${idir})
      set(include ${idir})
    elseif("${idir} " STREQUAL "include ")
      get_filename_component(include "${pcload_DIR}/../../../include" ABSOLUTE)
      if(NOT IS_DIRECTORY ${include})
        message(FATAL_ERROR "Project 'pcload' specifies '${idir}' as an include dir, which is not found.  It does not exist in '${include}'.  ${_report}")
      endif()
    else()
      message(FATAL_ERROR "Project 'pcload' specifies '${idir}' as an include dir, which is not found.  It does neither exist as an absolute directory nor in '/home/oem/pcd2pgm/pcload/src/pcload/${idir}'.  ${_report}")
    endif()
    _list_append_unique(pcload_INCLUDE_DIRS ${include})
  endforeach()
endif()
...
foreach(library ${libraries})
  ...
  if("${library}" MATCHES "^(debug|optimized|general)$")
    list(APPEND pcload_LIBRARIES ${library})
  elseif(${library} MATCHES "^-l")
    list(APPEND pcload_LIBRARIES ${library})
  elseif(${library} MATCHES "^-")
    # This is a linker flag/option (like -pthread)
    # There's no standard variable for these, so create an interface library to hold it
    if(NOT pcload_NUM_DUMMY_TARGETS)
      set(pcload_NUM_DUMMY_TARGETS 0)
    endif()
    # Make sure the target name is unique
    set(interface_target_name "catkin::pcload::wrapped-linker-option${pcload_NUM_DUMMY_TARGETS}")
    while(TARGET "${interface_target_name}")
      math(EXPR pcload_NUM_DUMMY_TARGETS "${pcload_NUM_DUMMY_TARGETS}+1")
      set(interface_target_name "catkin::pcload::wrapped-linker-option${pcload_NUM_DUMMY_TARGETS}")
    endwhile()
    ...
    list(APPEND pcload_LIBRARIES "${interface_target_name}")
  ...
  elseif(IS_ABSOLUTE ${library})
    ...
    foreach(path /home/oem/pcd2pgm/pcload/devel/lib;/opt/ros/noetic/lib)
    ...
    endforeach()
    if(lib)
      _list_append_unique(pcload_LIBRARY_DIRS ${lib_path})
      list(APPEND pcload_LIBRARIES ${lib})
    else()
      # as a fall back for non-catkin libraries try to search globally
      find_library(lib ${library})
      if(NOT lib)
        message(FATAL_ERROR "Project '${PROJECT_NAME}' tried to find library '${library}'.  The library is neither a target nor built/installed properly.  Did you compile project 'pcload'?  Did you find_package() it before the subdirectory containing its code is included?")
      endif()
      list(APPEND pcload_LIBRARIES ${lib})
    endif()
  endif()
...
set(pcload_EXPORTED_TARGETS "")
# create dummy targets for exported code generation targets to make life of users easier
foreach(t ${pcload_EXPORTED_TARGETS})
  if(NOT TARGET ${t})
    add_custom_target(${t})
  endif()
endforeach()

set(depends "roscpp;sensor_msgs;geometry_msgs;rosbag")
foreach(depend ${depends})
  string(REPLACE " " ";" depend_list ${depend})
  # the package name of the dependency must be kept in a unique variable so that it is not overwritten in recursive calls
  list(GET depend_list 0 pcload_dep)
  list(LENGTH depend_list count)
  if(${count} EQUAL 1)
    # simple dependencies must only be find_package()-ed once
    if(NOT ${pcload_dep}_FOUND)
      find_package(${pcload_dep} REQUIRED NO_MODULE)
    endif()
  else()
    # dependencies with components must be find_package()-ed again
    list(REMOVE_AT depend_list 0)
    find_package(${pcload_dep} REQUIRED NO_MODULE ${depend_list})
  endif()
  _list_append_unique(pcload_INCLUDE_DIRS ${${pcload_dep}_INCLUDE_DIRS})

  # merge build configuration keywords with library names to correctly deduplicate
  _pack_libraries_with_build_configuration(pcload_LIBRARIES ${pcload_LIBRARIES})
  _pack_libraries_with_build_configuration(_libraries ${${pcload_dep}_LIBRARIES})
  _list_append_deduplicate(pcload_LIBRARIES ${_libraries})
  # undo build configuration keyword merging after deduplication
  _unpack_libraries_with_build_configuration(pcload_LIBRARIES ${pcload_LIBRARIES})

  _list_append_unique(pcload_LIBRARY_DIRS ${${pcload_dep}_LIBRARY_DIRS})
  _list_append_deduplicate(pcload_EXPORTED_TARGETS ${${pcload_dep}_EXPORTED_TARGETS})
endforeach()

set(pkg_cfg_extras "")
foreach(extra ${pkg_cfg_extras})
  if(NOT IS_ABSOLUTE ${extra})
    set(extra ${pcload_DIR}/${extra})
  endif()
  include(${extra})
endforeach()
